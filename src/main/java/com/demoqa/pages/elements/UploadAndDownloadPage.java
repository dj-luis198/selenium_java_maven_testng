...
public class UploadAndDownloadPage extends BaseClass {
    ...
    public void clickDownloadButton() {
        BaseClass.click(downloadButton);
    }
    ...
    public Boolean verifyuploadedFilePath() {
        if (BaseClass.getTextContent(uploadedFilePath).contains(file.getName())) {
            return true;
        }
        return false;
    }
    ...
    public Boolean verifyDownloadedFilePath() {
        String expectedFileName = BaseClass.getAttributeDownload(downloadButton);
        String downloadPath = "files/downloadFiles";
        if(BaseClass.waitDownloadFile(expectedFileName,downloadPath,15)){
            clearFiles();
            return true;
        }
        clearFiles();
